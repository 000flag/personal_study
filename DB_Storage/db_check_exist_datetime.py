...
def get_date_range(cursor, db_name, table, datetime_col):
    try:
        # 해당 테이블에서 IS NOT NULL 조건을 추가하여 NULL 값, 비정상적인 값 제외하고 datetime 컬럼 값 중 가장 오래된 날짜(MIN), 가장 최신 날짜(MAX) 를 가져오기
        cursor.execute(f"""
            SELECT 
                MIN(`{datetime_col}`), 
                MAX(`{datetime_col}`)
            ...
            WHERE `{datetime_col}` IS NOT NULL 
                ...
                AND `{datetime_col}` >= '1900-01-01'
                AND `{datetime_col}` < '2100-01-01'
        """)
...
'''
    ...
    batch_size: 한 번에 조회할 row 수 (기본 5000)
'''
def estimate_storage(cursor, db_name, table, datetime_col, start, end, avg_len, batch_size=5000):
    total_count = 0 # 전체 행 수 누적용
    offset = 0 # 페이징 처리 시 사용
    ...
    while True:
        try:
            '''
                - 날짜 필터링
                    - >= start, < end: 지정된 기간 범위
                    - 비정상 값 (0000-00-00, 0000-00-00 00:00:00) 제외
                    - 유효한 범위 (1900년 이상, 2100년 미만)
                - 페이징 처리
                    - 한 번에 batch_size만큼 데이터를 가져오고, 계속 다음 OFFSET으로 진행
            '''
            cursor.execute(f"""
                SELECT `{datetime_col}`
                FROM `{db_name}`.`{table}`
            ...
            # 이번 페이지에서 가져온 row 수를 누적
            rows = cursor.fetchall()
            count = len(rows)
            total_count += count

            # 가져온 row 수가 batch_size보다 적으면 → 마지막 페이지 → 종료
            if count < batch_size:
                break

            offset += batch_size

        # 쿼리 실패 시 오류 출력 후 종료 (루프 중단)
        except Exception as e:
            print(f"[ERROR] 배치 처리 실패 - {table} ({start} ~ {end}) - {e}")
            break

    '''
        - 계산식
            총 행 수 X 평균 행 크기(byte) = 전체 크기(byte)
            바이트 → 메가바이트: / 1024 / 1024
            소수점 2자리로 반올림
    '''
    estimated_mb = total_count * avg_len / 1024 / 1024
